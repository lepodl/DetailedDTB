    ...
    def __init__(self, block_dict: dict, bold_dict: dict, steps=800, ensembles=100, time=400, hp_sigma=0.01,
       ...
       bold_dict : dict
        ...
        self.block = block_gpu(block_dict['ip'], block_dict['block_path'],
                               block_dict['delta_t'], route_path=None, overlap=10)
        self.block.update_ou_background_stimuli(torch.tensor([4]).type(torch.float32).cuda(),
                                                      torch.tensor([0.4]).type(torch.float32).cuda(),
                                                      torch.tensor([0.15]).type(torch.float32).cuda())
        # self.block.update_ou_background_stimuli(torch.tensor([10]).type(torch.float32).cuda(),
        #                                               torch.tensor([0.7]).type(torch.float32).cuda(),
        #                                               torch.tensor([0.3]).type(torch.float32).cuda())
...
class DA_Rest_Thalamus(DA_MODEL):
    def __init__(self, block_dict: dict, bold_dict: dict, whole_brain_voxel_info: str, steps, ensembles, time, hp_sigma,
        ...
        brain_file = h5py.File(whole_brain_voxel_info, "r")
        self.device = "cuda:0"
        self.HCPex_region = np.array(brain_file['NSR_dti_HCPex_label']).squeeze() # region index from 1
        self.num_voxel_in_one_ensemble = len(self.HCPex_region)
        self.num_cortical_voxel_in_one_ensemble = 14473
        ...
        self.neurons_per_voxel_subcortical = self.block.neurons_per_subblk.float()[
                                          self.num_cortical_voxel_in_one_ensemble * 8:self.num_populations_in_one_ensemble].reshape((-1, 2)).sum(axis=1)
    ...
    def update_hp(self, property_index=torch.tensor([10, 11, 12, 13])):
        # 注意，原来property_index=torch.tensor([11, 12, 13])，现在改为property_index=torch.tensor([10, 11, 12, 13])

        print("cortical_population_id", self.cortical_population_id[:10], self.cortical_population_id[-10:])
        print("subcortical_population_id", self.subcortical_population_id[:10], self.subcortical_population_id[-10:])
        gui = np.array([0.002, 2.5e-05, 0.0091, 0])  # 静息态 ampa 游走上下界：0.00001，0.00023
        ...
        for i in range(len(property_index)):
            self.block.assign_property_by_subblk(torch.stack(
                torch.meshgrid(self.cortical_population_id, property_index[i].type_as(self.type_int).reshape(-1)),
                dim=-1).reshape(-1, 2).type_as(self.type_int), gui_cortical_ones_else.type_as(self.type_float).reshape(-1) * gui[i])
            self.block.assign_property_by_subblk(torch.stack(
                torch.meshgrid(self.subcortical_population_id, property_index[i].type_as(self.type_int).reshape(-1)),
                dim=-1).reshape(-1, 2).type_as(self.type_int), gui_subcortical_ones_else.type_as(self.type_float).reshape(-1) * gui[i])

    def conditional_info_helper(self, region=(42, 43), excited=True):
        region = np.array(region)
        if not excited:
            raise NotImplementedError
        voxels_belong_to_region = np.isin(self.HCPex_region, region).nonzero()[0]
    ...
    def gamma_initialize(self, hp_index, population_id=None, alpha=5., beta=5.):
        ...
        out = self.block.gamma_property_by_subblk(population_info, alpha, beta, debug=False)
    ...
    def initial_model(self, mean_hp_after_da, para_ind=None):
        ...
        self.low_bound = np.ones((self.num_external_current_voxels, self.hp_num), dtype=np.float32) * 0.0  # 这里要改，先做模拟
        self.up_bound = np.ones((self.num_external_current_voxels, self.hp_num), dtype=np.float32) * 0.32  # 这里要改

        self.hp = np.linspace(self.low_bound, self.up_bound, num=3 * self.ensembles, dtype=np.float32)[
                  self.ensembles: - self.ensembles]
        self.hp = torch.from_numpy(self.hp).cuda()  # ensembles, num_voxel, 1
    ...
    def evolve(self, steps=800):
        ...
        wjx_act_subcortical_record = torch.zeros(steps, self.ensembles * len(self.extern_current_voxel))
        i=0

        steps = steps if steps is not None else self.steps
        for return_info in self.block.run(steps, freq_char=True, freqs=True, vmean=False, sample_for_show=False, iou=True, t_steps=10):
            act, *others = return_info

            # act_cortical = torch.empty(size=(self.ensembles, self.num_cortical_voxel_in_one_ensemble, 8),
            #                            device=self.device)
            act_subcortical = torch.empty(size=(self.ensembles, self.num_subcortical_voxel_in_one_ensemble, 2),
                                          device=self.device)

            for idx in range(self.ensembles):
                s = idx * self.num_populations_in_one_ensemble
                e = s + self.num_cortical_voxel_in_one_ensemble * 8
                e_ = (idx + 1) * self.num_populations_in_one_ensemble
                # act_cortical[idx] = act.float()[s: e].reshape((self.num_cortical_voxel_in_one_ensemble, 8))
                act_subcortical[idx] = act.float()[e: e_].reshape((self.num_subcortical_voxel_in_one_ensemble, 2))

            act_subcortical_voxel = (act_subcortical.sum(axis=-1) / self.neurons_per_voxel_subcortical)[:,
                                 self.extern_current_voxel - self.num_cortical_voxel_in_one_ensemble].reshape(-1)
            out1 = self.bold_cortical.run(torch.max(act_subcortical_voxel, torch.tensor([1e-05]).type_as(act_subcortical)))

            wjx_act_subcortical_record[i, :] = act_subcortical_voxel
            i = i+1

        print(
            f'\n\n\n\n   cortical active: mean: {wjx_act_subcortical_record.mean(axis=0).mean().item()}, \nmin: {torch.min(wjx_act_subcortical_record.mean(axis=0))}, \nmax: {torch.max(wjx_act_subcortical_record.mean(axis=0))}'
            f', \n[:8]: {wjx_act_subcortical_record.mean(axis=0)[:8]}, \n[8:16]: {wjx_act_subcortical_record.mean(axis=0)[8:16]}, \n[16:24]: {wjx_act_subcortical_record.mean(axis=0)[16:24]}')
        # print(
        ...
        w = torch.cat(
            (
                self.hp_log, act_subcortical_voxel.reshape([self.ensembles, -1, 1]),
    ...
    def run(self, hp_after_da_path, da_para_ind, bold_path, write_path):
        ...
        tb.add_row([4, "voxels_per_ensemble", self.num_voxel_in_one_ensemble, "populations_per_voxel", "1 & 1"])
        tb.add_row([5, "total_time", self.T, "steps", self.steps])
        tb.add_row([6, "hp_sigma", self.hp_sigma, "bold_sigma", self.bold_sigma])
        tb.add_row([7, "noise_rate", self.noise_rate, "extern_number", self.num_external_current_voxels])
        tb.add_row([8, "initial_steps", 800, "bold_range", "0.046-0.076"])
        tb.add_row([9, "rate", 0.5, "rate", 0.5])
        tb.add_row([10, "walk_upbound", "0.32", "walk_low_bound", "0."])
        print(tb)
        ...
        for t in range(self.T - 1):
            ...
            print("Hp_Cortical, eg: ", self.hp[0, 0, :1].data)
...
if __name__ == '__main__':
    ...
    bold_dict = {"epsilon": 200,
                 ...
                 "E_0": 0.8,
                 "V_0": 0.02}
    # real_parameter = {"cortical": np.array([0.000812, 0.001508, 0.000928, 0.001624, 0.000812, 0.00174,
    #    0.000812, 0.001972]).reshape(-1, 1),
    #                   "subcortical": np.array([0.001008, 0.001008]).reshape(-1, 1)}
    # real_parameter = {"cortical": np.array([0.0007, 0.0013, 0.0008, 0.0014, 0.0007, 0.0015,
    #                                         0.0007, 0.0017]).reshape(-1, 1),
    #              "subcortical": np.array([0.0008, 0.0008]).reshape(-1, 1)}
    ...
    block_dict.update(
        {"ip": args.ip, "block_path": args.block_path, "noise_rate": args.noise_rate})

    whole_brain_voxel_info_path ="/public/home/ssct004t/project/zenglb/DetailedDTB/data/raw_data/NSR_data_May22.mat"

    da_task = DA_Rest_Thalamus(block_dict, bold_dict, whole_brain_voxel_info_path, steps=args.steps,
    ...
    da_task.run(hp_after_da_path=None
                ...
                bold_path="", write_path=args.write_path) # TODO
